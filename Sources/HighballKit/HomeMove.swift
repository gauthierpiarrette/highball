...
/// the real home folders; `FileManager.copyItem` is the wrong tool for that tree. On a network
/// share it is worse: it also copies ACLs, `com.apple.provenance` and Spotlight's
/// `.metadata_never_index` marker, and SMB commonly refuses those — the failure reads as
/// ".metadata_never_index couldn’t be copied because you don’t have permission to access
/// drive_c". Data and the link structure are what an environment needs; Apple extra files are
/// not, and a new marker is planted at the destination so Spotlight stays out.
///
/// File bytes go through `read`/`write` whenever the destination is not the same local volume.
/// `fclonefileat` (and `fcopyfile`) on smbfs can return success or hang without sending a byte —
/// macOS asks the server to clone, the source is this Mac, and the share sits idle while the
/// strip still says "Copying…".
