...
final class AppState {
    ...
    /// Settings › Environments › Change…: a folder Highball can actually hold an environment in,
    /// checked before anything moves (symlinks, colon names like `c:`, execute, file size).
    func chooseHome() {
        let panel = NSOpenPanel()
        ...
        runBusy(String(format: L("Moving Highball's data to %@"), target.lastPathComponent),
            ...
            try await Task.detached {
                try HomeMove.move(from: source, to: target) { name, copied, items in
                    Task { @MainActor in
                        self.reportHomeCopy(name: name, copied: copied, items: items)
                    }
                }
