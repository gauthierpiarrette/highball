...
enum L10n {
    static let fr: [String: String] = [
        ...
        "Got it": "Compris",
        "Adaptive pacing (fill the display rate, up to the multiplier)": "Cadence adaptative (remplir la fréquence de l’écran, jusqu’au multiplicateur)",
        "Performance mode (cheaper generation, lower quality)": "Mode performance (génération moins coûteuse, qualité moindre)",
        ...
        "Lossless Scaling inserts interpolated frames between the game's real frames to make motion look smoother. The generated frames are paced to your display, so it can never present more frames than your screen's refresh rate.\n\nThat means it only helps when a game runs BELOW your refresh rate, for example a demanding game locked at 30 fps smoothed up to 60. On the built-in 60 Hz display, a game already running above 60 gets capped to 60 and feels worse, not better.\n\nIt pays off on an external high-refresh monitor (120 Hz or more), where a 60 fps game becomes 120. It also adds a little input lag, because a real frame is held back to interpolate.\n\nAdaptive pacing measures the game's own frame rate and inserts only as many frames as it takes to fill the display, up to the multiplier: a 45 fps game gets one extra frame every other frame, and a game already at the refresh rate is left alone. It works on every graphics mode.\n\nFrame generation works with every graphics mode: DXVK and vkd3d-proton through Vulkan, DXMT and D3DMetal through Metal, and WineD3D by switching it to its Vulkan renderer. After changing it, stop the environment and relaunch so a Steam game picks up the new setting.\n\nThis is a beta feature: some games may show artifacts, stutter or not start with it on. Turn it off if a game misbehaves.": "Lossless Scaling insère des images interpolées entre les vraies images du jeu pour rendre le mouvement plus fluide. Les images générées sont cadencées sur votre écran : elles ne peuvent jamais dépasser sa fréquence de rafraîchissement.\n\nElle n’aide donc que lorsqu’un jeu tourne EN DESSOUS de votre fréquence, par exemple un jeu exigeant bloqué à 30 fps lissé jusqu’à 60. Sur l’écran intégré 60 Hz, un jeu déjà au-dessus de 60 est plafonné à 60 et paraît moins bon, pas meilleur.\n\nElle est utile sur un moniteur externe à haute fréquence (120 Hz ou plus), où un jeu à 60 fps passe à 120. Elle ajoute aussi un peu de latence, car une vraie image est retenue pour l’interpolation.\n\nLa cadence adaptative mesure la fréquence propre du jeu et n’insère que les images nécessaires pour remplir l’écran, jusqu’au multiplicateur : un jeu à 45 fps reçoit une image supplémentaire une fois sur deux, et un jeu déjà à la fréquence de l’écran est laissé tel quel. Elle fonctionne avec tous les modes graphiques.\n\nLa génération d’images fonctionne avec tous les modes graphiques : DXVK et vkd3d-proton via Vulkan, DXMT et D3DMetal via Metal, et WineD3D en le basculant sur son moteur de rendu Vulkan. Après l’avoir changée, arrêtez l’environnement et relancez pour qu’un jeu Steam applique le nouveau réglage.\n\nCette fonction est en bêta : certains jeux peuvent présenter des artefacts, des saccades ou ne pas démarrer avec elle. Désactivez-la si un jeu se comporte mal.",
