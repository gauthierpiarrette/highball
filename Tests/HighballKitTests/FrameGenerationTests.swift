...
private func frameGenerationChecks() throws {
    ...
    try check(env["LSFGM_ENV"] == "1" && env["LSFGM_METAL"] == nil, "WineD3D did not use the Vulkan hook")
    ...
    try check(env["LSFGM_METAL"] == nil && env["DYLD_INSERT_LIBRARIES"] == nil, "Vulkan renderer picked up the metal hook")
    ...
    env = try bottle.environment(engine: engine, renderer: .dxmt, extra: ["LSFGM_METAL": "1"])
    try check(env["LSFGM_METAL"] == nil && env["DISABLE_LSFGM"] == "1", "Off setting left the metal hook armed")
